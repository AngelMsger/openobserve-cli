// Package credstore persists secrets for the OpenObserve tools. It prefers the
// OS keychain and transparently falls back to a protected file under the config
// directory when no keychain is available.
//
// It lives in pkg/ because the CLI and the o3 desktop app must agree byte for
// byte on where a credential is kept: they share one keychain service name and
// one fallback file, so a credential saved by either is found by the other. o3
// previously called the keychain directly with no fallback, which let the two
// diverge on hosts without a usable keychain.
package credstore
...
// Backend is the OS keychain a Store writes to. Exported so callers can supply
// a stub in tests; NewStore wires the real one.
...
// Store persists secrets, preferring the OS keychain over its file fallback.
// Windows encrypts the fallback with per-user DPAPI; other platforms write a
// 0600 file.
type Store struct {
...
func NewStore(dir string) *Store { return NewStoreWithBackend(dir, systemKeyring{}) }
...
func NewStoreWithBackend(dir string, b Backend) *Store {
	return &Store{dir: dir, keyring: b}
}
...
func (s *Store) Load(account string) (string, error) {
	keyringErr := error(ErrSecretNotFound)
	if s.useKeychain() {
		var secret string
	...
	keyringMissing := errors.Is(keyringErr, keyring.ErrNotFound) || errors.Is(keyringErr, ErrSecretNotFound)
