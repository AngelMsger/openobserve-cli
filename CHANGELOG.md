...
- Credential fallback tests now check POSIX permissions only where supported
  and verify Windows DPAPI protection through the complete store save/load path.
  This fixes the Windows CI assertion without changing credential storage.

## [0.11.3] - 2026-09-16

### Skill

- Correct shell error recovery to preserve the original exit status. Narrow
  discovery to unknown identifiers, keep incident windows fixed, and separate
  finite inspection from bounded live monitoring. Clarify pagination budgets
  and evidence-based reporting without dumping raw logs. Skill bumped to `0.3.3`.
...
- New `pkg/credstore` package: the credential store (OS keychain with a
  protected-file fallback) moved out of `internal/` so the o3 desktop app can
  use the same implementation. Both tools already share one keychain service
  name and one fallback file; a second implementation would let them diverge on
  hosts without a usable keychain.

### Fixed

- **macOS: never provoke the "Keychain Not Found" system dialog.** Storing a
  secret ran `security add-generic-password`, and when the process had no
  default login keychain the Security framework put up a modal whose default
  button — "Reset To Defaults" — resets the user's default keychain. The command
  then exited 154, surfacing as an opaque "exit status 154". The store now asks
  `security default-keychain` first, which answers the same question with no UI,
  and routes straight to the file fallback when there is no keychain to write
  to. No application should be able to raise that dialog.
...
[Unreleased]: https://github.com/AngelMsger/openobserve-cli/compare/v0.11.4...HEAD
