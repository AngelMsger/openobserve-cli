...
func loginStateForTest(t *testing.T) (*appState, config.Config, auth.Credential) {
	t.Helper()
	dir := t.TempDir()
	cfg := config.Config{BaseURL: "https://service.example.test/deploy", Auth: config.AuthConfig{Scheme: "basic", CredentialURL: "https://help.example.test/credentials"}, Org: "team-org"}
...
func TestVerifyLoginRejectsAnonymousResponses(t *testing.T) {
