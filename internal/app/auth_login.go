...
func loginFile(s *appState, cfg config.Config, cred auth.Credential) (config.File, string, error) {
	...
	if !found {
		nc = config.NamedContext{Name: name, Org: cfg.Org}
	}
	if nc.BaseURL == "" {
...
func verifyCredential(s *appState, cfg config.Config, cred auth.Credential) error {
	...
	defer cancel()
	client, err := apiclient.Build(apiclient.BuildParams{BaseURL: cfg.BaseURL, Org: cfg.Org, AuthDecorator: cred.Decorator(), Timeout: s.timeout(), MaxRetries: cfg.Defaults.MaxRetries})
	if err != nil {
		return err
	}
