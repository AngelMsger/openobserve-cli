...
func setupFixture() NamedContext {
	return NamedContext{Name: "team", BaseURL: "https://service.example.test/deploy", Auth: AuthConfig{Scheme: "token", Username: "PersonalUser", CredentialURL: "https://help.example.test/tokens"}, Org: "team-org"}
}
...
func TestSetupTargetsNamedContextAndExcludesPersonalEnvironment(t *testing.T) {
	...
	dir := t.TempDir()
	target := setupFixture()
	file := File{CurrentContext: "other", Contexts: []NamedContext{{Name: "other", BaseURL: "https://other.example.test", Auth: AuthConfig{Scheme: "token"}}, target}}
	if err := WriteFile(dir, file); err != nil {
		t.Fatal(err)
	}
	t.Setenv("OPENOBSERVE_CONTEXT", "not-defined")
	t.Setenv("OPENOBSERVE_FORMAT", "ndjson")
	t.Setenv("OPENOBSERVE_EMAIL", "InjectedUser")
	t.Setenv("OPENOBSERVE_PASSWORD", "must-not-be-copied")
	dotenv := filepath.Join(dir, "presets.env")
	if err := os.WriteFile(dotenv, []byte("OPENOBSERVE_URL=https://dotenv.example.test/deploy\nOPENOBSERVE_CREDENTIAL_URL=https://help.example.test/new\nOPENOBSERVE_PASSWORD=dotenv-secret\nOPENOBSERVE_EMAIL=dotenv-user\n"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if got.ActiveContext != "team" || got.Config.BaseURL != "https://dotenv.example.test/deploy" || got.Config.Auth.Username != "PersonalUser" || got.Config.Auth.Scheme != "token" {
	...
	t.Setenv("OPENOBSERVE_URL", "https://env.example.test")
	t.Setenv("OPENOBSERVE_AUTH_SCHEME", "basic")
	got, err = Load(opt)
	if err != nil {
		t.Fatal(err)
	}
	...
	opt.Flags = FlagValues{BaseURL: "https://flag.example.test", CredentialURL: "https://help.example.test/flag", AuthScheme: "token"}
	got, err = Load(opt)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCredentialGuideIsDisplayOnlyAndKeepsDeploymentPath(t *testing.T) {
	nc := setupFixture()
	cfg := Config{BaseURL: nc.BaseURL, Auth: nc.Auth, Org: "team-org"}
	cfg.Auth.CredentialURL = ""
	g, err := Guide(cfg, nil)
	if err != nil {
		t.Fatal(err)
	}
	if g.CredentialURL != nc.BaseURL {
